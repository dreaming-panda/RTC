...
class Pipeline:
    ...
    async def get_frame_item(self) -> FrameItem:
        try:
            ...
            item = FrameItem(img=img, infer_id=0, frame_index=0, last_action=None)
            self._last_frame = item
            return item
    ...
    async def _run(self):
        while not self._stop.is_set():
            actions: List[Action] = []
            for _ in range(ACTIONS_PER_INFER):
                a = await self.action_q.get()
                actions.append(a)
            ...
            for idx, img in enumerate(frames):
                ...
                item = FrameItem(
                    ...
                    infer_id=infer_id,
                    frame_index=idx,
                    last_action=(last_action if idx == 0 else None),
                )
...
class PipelineVideoTrack(VideoStreamTrack):
    ...
    async def recv(self):
        ...
        if (
            ...
            and item.last_action is not None
        ):
